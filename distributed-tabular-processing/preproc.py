...
class PreprocessDF():
    ...
    def preproc_dataframe(self, gdf, mode):
        start = time()
        self.gdf = gdf
        self.mode = mode
        self.categorify()
        ...
        if self.mode == TRAIN:
            ...
            def _fillify(y):
                if self.fill_strategy == MEDIAN:
                    self.filler = {name: self.get_median(y[name]) for name in y.columns}
                elif self.fill_strategy == CONSTANT:
                    self.filler = {name: self.fill_val for name in y.columns}
                else:
                    self.filler = {name: y[name].value_counts().index[0] for name in y.columns}
                if self.add_col: 
                    self.add_col_(y.columns)
                return y.fillna(self.filler, inplace=True)
            
            self.gdf[self.train_cont_names_na] = self.gdf[self.train_cont_names_na].map_partitions(
                _fillify
            )
        ...
        for name in self.cat_names:
            if False:
                if self.mode == TRAIN:
                    ...
                    self.gdf[name] = self.gdf[name].map_partitions(self.category_encoders[name].fit_transform, meta=cudf.Series([0]))
                    # self.gdf[name] = self.category_encoders[name].fit_transform(self.gdf[name].append([None]))[:-1]
                else:
                    self.gdf[name] = self.category_encoders[name].transform(self.gdf[name].append([None]))[:-1]
                self.gdf[name] = self.gdf[name].astype('int64')
            else:
                def _hashify(s):
                    return s.hash_values().astype('int64')
                self.gdf[name] = self.gdf[name].map_partitions(_hashify, meta=cudf.Series([0]))